...
class ConvertByHeating(Action):
    """ heat container """

    container_iri: str
    """ the iri of the container """

    heating_module_iri: str
    """ the iri of the heating module """

    conversion_ratio: float
    """ ratio of the conversion (yield) """

    reaction: ChemicalReaction

    def get_resources(self) -> list[str]:
        """ a list of iris of the resources, they are assumed to be `LabObject` instances """
        resources = [self.container_iri, self.heating_module_iri]
    ...
    def get_action_effects(self) -> list[UnitaryEdit]:
        ...
        for c in self.reaction.reagents:
            pom_after.has_ingredient.add(c.model_dump_json())
